...
def upload_page():
    ...
    if uploaded_file and st.button("Share", type="primary"):
        with st.spinner("Uploading..."):
            ...
            data = {"caption": caption, "file_type": uploaded_file.type, "file_name": uploaded_file.name}
            headers = get_headers()
            headers.pop("Content-Type", None)  # Let requests set the correct Content-Type for multipart
            response = requests.post("http://127.0.0.1:8000/upload", files=files, data=data, headers=headers)
